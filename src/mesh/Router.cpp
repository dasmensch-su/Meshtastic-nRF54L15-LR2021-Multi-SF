...
ErrorCode Router::sendLocal(meshtastic_MeshPacket *p, RxSource src)
{
    ...
    if (isToUs(p)) {
        ...
        LOG_DEBUG("sendLocal: calling send() for to=0x%08X", p->to);
        auto rc = send(p);
        LOG_DEBUG("sendLocal: send() returned %d", (int)rc);
        return rc;
    }
}
/**
...
#if defined(MESHTASTIC_MULTI_SF_BRIDGE) && MESHTASTIC_MULTI_SF_BRIDGE
    /* Multi-SF bridge: if the normal channels[] loop failed, try the shadow
     * table of preset-derived (name, default_PSK) hashes. Lets the bridge
     * decrypt packets stamped with ANY standard preset's channel hash as
     * long as the peer used the default PSK. See src/mesh/ShadowChannels.h
     * for the full rationale and limitations. This block is a no-op when
     * Multi-SF is compiled out. */
    if (!decrypted) {
        extern bool shadowChannelsTryDecrypt(meshtastic_MeshPacket *p);
        if (shadowChannelsTryDecrypt(p)) {
            decrypted = true;
            chIndex = 0;  /* associate shadow-decoded packets with the primary
                           * channel index; the hash won't match any real slot,
                           * but downstream only uses this for display/MQTT
                           * and the primary is the closest logical fit. */
        }
    }
#endif
...
#if defined(MESHTASTIC_MULTI_SF_BRIDGE) && MESHTASTIC_MULTI_SF_BRIDGE
            /* Fix #1: per-peer outbound channel-hash override. If we've
             * shadow-decoded packets from this unicast destination we know
             * the wire hash their stock firmware expects to see on ITS own
             * channel. Stamp that hash in place of our own so the peer's
             * channel filter accepts the packet. Safe because the payload
             * is still AES-CTR encrypted with the (shared) default PSK — we
             * are only changing a dispatch hint, not cryptography. */
            if (!isBroadcast(p->to) && isFromUs(p)) {
                uint8_t peerHash = nodeSFTracker.getHash(p->to);
                if (peerHash != 0xFF && peerHash != (uint8_t)hash) {
                    LOG_INFO("Cross-preset hash override 0x%02x -> 0x%02x for 0x%08x",
                             (unsigned)hash, (unsigned)peerHash, (unsigned)p->to);
                    p->channel = peerHash;
                }
            }
#endif
...
#if defined(MESHTASTIC_MULTI_SF_BRIDGE) && MESHTASTIC_MULTI_SF_BRIDGE
        /* Fix #1: see the PKI-enabled branch above — identical logic. */
        if (!isBroadcast(p->to) && isFromUs(p)) {
            uint8_t peerHash = nodeSFTracker.getHash(p->to);
            if (peerHash != 0xFF && peerHash != (uint8_t)hash) {
                LOG_INFO("Cross-preset hash override 0x%02x -> 0x%02x for 0x%08x",
...
void Router::handleReceived(meshtastic_MeshPacket *p, RxSource src)
{
    ...
    if (!skipHandle) {
        LOG_DEBUG("About to callModules for id=0x%08X", p->id);
        MeshModule::callModules(*p, src);
        LOG_DEBUG("callModules returned for id=0x%08X", p->id);
