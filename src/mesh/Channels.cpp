...
meshtastic_Channel &Channels::fixupChannel(ChannelIndex chIndex)
{
    ...
    if (!ch.has_settings) {
        ...
        // Match case-insensitively: stock Adafruit Muzi firmware and older
        // XIAO-port NVS have been observed with lowercase "default", which
        // upstream's exact-match strcmp() would miss. Leaving the name as
        // "default" changes xor(name) and therefore the channel-hash byte,
        // which makes otherwise-identical default-channel nodes filter
        // each other's packets. Normalising both variants to "" here lets
        // the preset-name substitution take over consistently and keeps
        // the wire hash aligned across vendors.
        if (strcasecmp(meshtastic_channelSettings.name, "Default") == 0)
