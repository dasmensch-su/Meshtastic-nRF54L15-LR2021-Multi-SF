...
int32_t NextHopRouter::doRetransmissions()
{
    ...
    /* Collect failed entries to process AFTER iteration — sendAckNak/stopRetransmission
     * can re-enter the pending map via sendLocal→sniffReceived, corrupting iterators. */
    struct FailedEntry { NodeNum keyNode; PacketId keyId; NodeNum from; PacketId id; ChannelIndex ch; bool needsNak; };
    FailedEntry failedEntries[8];
    int failedCount = 0;
    ...
    for (auto it = pending.begin(), nextIt = it; it != pending.end(); it = nextIt) {
        ...
        if (p.nextTxMsec <= now) {
            if (p.numRetransmissions == 0) {
                /* Defer NAK + stop until after loop to avoid re-entrancy crash */
                if (failedCount < 8) {
                    failedEntries[failedCount].keyNode = it->first.node;
                    failedEntries[failedCount].keyId = it->first.id;
                    failedEntries[failedCount].from = getFrom(p.packet);
                    failedEntries[failedCount].id = p.packet->id;
                    failedEntries[failedCount].ch = p.packet->channel;
                    failedEntries[failedCount].needsNak = isFromUs(p.packet);
                    failedCount++;
                }
                stillValid = false; // will be deleted after loop
            } else {
    ...
    /* Process deferred failures now that iteration is done */
    for (int i = 0; i < failedCount; i++) {
        if (failedEntries[i].needsNak) {
            LOG_DEBUG("Reliable send failed, returning a nak for fr=0x%x,to=0x%x,id=0x%x",
                      failedEntries[i].from, 0xFFFFFFFF, failedEntries[i].id);
            sendAckNak(meshtastic_Routing_Error_MAX_RETRANSMIT,
                       failedEntries[i].from, failedEntries[i].id, failedEntries[i].ch);
        }
        stopRetransmission(GlobalPacketId(failedEntries[i].keyNode, failedEntries[i].keyId));
    }
