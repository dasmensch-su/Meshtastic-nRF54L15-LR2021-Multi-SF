...
{
    LOG_DEBUG("sendQueueStatusToPhone: allocating from queueStatusPool");
    meshtastic_QueueStatus *copied = queueStatusPool.allocCopy(qs);
    LOG_DEBUG("sendQueueStatusToPhone: allocCopy returned %p", copied);
    if (!copied) return ERRNO_UNKNOWN;

    LOG_DEBUG("sendQueueStatusToPhone: setting fields");
    copied->res = res;
    ...
    LOG_DEBUG("sendQueueStatusToPhone: enqueueing to toPhoneQueueStatusQueue");
    lastQueueStatus = *copied;

    res = toPhoneQueueStatusQueue.enqueue(copied, 0);
    LOG_DEBUG("sendQueueStatusToPhone: enqueue returned, fromNum++");
    fromNum++;
...
void MeshService::sendToMesh(meshtastic_MeshPacket *p, RxSource src, bool ccToPhone)
{
    ...
    LOG_DEBUG("sendToMesh: sendLocal returned %d", res);
