/*
 * aes_impl.c — Compact AES-128/256 ECB implementation for nRF54L15.
 *
 * Based on the public-domain tiny-AES-c by kokke (https://github.com/kokke/tiny-AES-c).
 * Provides aes_ecb_encrypt() used by our AES.h shim for Meshtastic CryptoEngine.
 *
 * For hardware deployment, replace with CRACEN or mbedTLS.
 */
#include <stdint.h>
#include <string.h>

/* S-box and inverse S-box */
static const uint8_t sbox[256] = {
  
};

static const uint8_t Rcon[11] = {
  0x8d,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36
};

#define xtime(x) ((uint8_t)(((x)<<1) ^ ((((x)>>7) & 1) * 0x1b)))

static void SubBytes(uint8_t *state)   { for (int i=0;i<16;i++) state[i]=sbox[state[i]]; }
static void ShiftRows(uint8_t *s) {
    uint8_t t;
    t=s[1]; s[1]=s[5]; s[5]=s[9]; s[9]=s[13]; s[13]=t;
    t=s[2]; s[2]=s[10]; s[10]=t; t=s[6]; s[6]=s[14]; s[14]=t;
    t=s[15]; s[15]=s[11]; s[11]=s[7]; s[7]=s[3]; s[3]=t;
}
static void MixColumns(uint8_t *s) {
    for (int i=0;i<16;i+=4) {
        uint8_t a=s[i],b=s[i+1],c=s[i+2],d=s[i+3],e=a^b^c^d;
        s[i]^=e^xtime(a^b); s[i+1]^=e^xtime(b^c);
        s[i+2]^=e^xtime(c^d); s[i+3]^=e^xtime(d^a);
    }
}
static void AddRoundKey(uint8_t *state, const uint8_t *rk) {
    for (int i=0;i<16;i++) state[i]^=rk[i];
}

/* Key expansion for AES-128 (Nk=4, Nr=10) and AES-256 (Nk=8, Nr=14) */
void aes_key_expansion(const uint8_t *key, uint8_t *roundKeys, int keyLen)
{
    int Nk = keyLen / 4;
    int Nr = (Nk == 4) ? 10 : 14;
    int rkSize = 16 * (Nr + 1);

    memcpy(roundKeys, key, keyLen);
    int i = Nk;
    while (i < 4*(Nr+1)) {
        uint8_t temp[4];
        memcpy(temp, roundKeys + (i-1)*4, 4);
        if (i % Nk == 0) {
            uint8_t t = temp[0];
            temp[0]=sbox[temp[1]]^Rcon[i/Nk];
            temp[1]=sbox[temp[2]]; temp[2]=sbox[temp[3]]; temp[3]=sbox[t];
        } else if (Nk > 6 && i % Nk == 4) {
            for (int j=0;j<4;j++) temp[j]=sbox[temp[j]];
        }
        for (int j=0;j<4;j++)
            roundKeys[i*4+j] = roundKeys[(i-Nk)*4+j] ^ temp[j];
        i++;
    }
}

/* AES ECB encrypt a single 16-byte block */
void aes_ecb_encrypt(const uint8_t *roundKeys, int Nr, const uint8_t *input, uint8_t *output)
{
    uint8_t state[16];
    memcpy(state, input, 16);
    AddRoundKey(state, roundKeys);
    for (int round = 1; round < Nr; round++) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, roundKeys + round*16);
    }
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, roundKeys + Nr*16);
    memcpy(output, state, 16);
}
