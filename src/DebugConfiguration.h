...
/* On nRF54L15 Zephyr we do not compile SerialConsole.  LOG_* macros are
 * defined below using printk instead of the SerialConsole/DEBUG_PORT path. */
...
/* DEBUG_PORT now references the real SerialConsole instance on every
 * arch including nRF54L15 — SerialConsole + RedirectablePrint are now
 * compiled in (see src/platform/nrf54l15/Print.h + Stream.h shims and
 * the SerialConsole entries in zephyr/CMakeLists.txt). The LOG_*
 * macros below still route to printk on nRF54L15 for log messages,
 * but rpInit() / setDestination() / etc need the real Print object. */
#define DEBUG_PORT (*console) // Serial debug port

#ifdef ARCH_NRF54L15

/* Zephyr nRF54L15: LOG_* via printk (no SerialConsole) */
