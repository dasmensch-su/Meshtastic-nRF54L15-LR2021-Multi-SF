...
int16_t LR2021::setPaConfig(uint8_t pa, uint8_t paLfMode, uint8_t paLfDutyCycle, uint8_t paLfSlices, uint8_t paHfDutyCycle) {
  /* Semtech LR20xx command 0x0202: 3 packed data bytes
   * [0] = (pa_sel << 7) | (pa_lf_mode & 0x03)
   * [1] = (pa_lf_duty_cycle & 0xF0) | (pa_lf_slices & 0x0F)
   *        Note: duty_cycle constants are pre-shifted to upper nibble
   * [2] = pa_hf_duty_cycle & 0x1F */
  uint8_t buff[] = {
    (uint8_t)((pa << 7) | (paLfMode & 0x03)),
    (uint8_t)((paLfDutyCycle & 0xF0) | (paLfSlices & 0x0F)),
