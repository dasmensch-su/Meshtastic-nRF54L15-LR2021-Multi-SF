...
int16_t LR2021::stageMode(RadioModeType_t mode, RadioModeConfig_t* cfg) {
  ...
  switch(mode) {
    case(RADIOLIB_RADIO_MODE_RX): {
      ...
      // reset LoRa packet params for RX — the TX path sets payload length to
      // the actual packet size, which must be reset to max for proper reception
      if(modem == RADIOLIB_LR2021_PACKET_TYPE_LORA) {
        uint8_t rxLen = (this->headerType == RADIOLIB_LR2021_LORA_HEADER_IMPLICIT) ?
                        this->implicitLen : RADIOLIB_LR2021_MAX_PACKET_LENGTH;
        state = setLoRaPacketParams(this->preambleLengthLoRa, this->headerType, rxLen, this->crcTypeLoRa, this->invertIQEnabled);
        RADIOLIB_ASSERT(state);
      }
