...
class LR2021: public LRxxxx {
  public:
    ...
    */
    ...
    /*!
      ...
      \param detector One-hot flag (4-bit) of which detector received the packet: 0001=Main, 0010=Side1, 0100=Side2, 1000=Side3
      \returns \ref status_codes
    */
  ...
  private:
#endif
    // Cutoff between sub-GHz (LF) and 2.4 GHz (HF) RF paths (MHz).
    static constexpr float LF_HF_CUTOFF_MHZ = 1000.0f;

    // flag to determine whether we are in the sub-GHz or 2.4 GHz range
    // this is needed to automatically detect which PA and RX path to use
    bool highFreq = false;
