...
int16_t LRxxxx::reset() {
  ...
  // wait for BUSY to go low (use module timeout, default 1s may be too short for TCXO boards)
  RadioLibTime_t start = this->mod->hal->millis();
  RadioLibTime_t busyTimeout = this->mod->spiConfig.timeout > 3000 ? this->mod->spiConfig.timeout : 3000;
  while(this->mod->hal->digitalRead(this->mod->getGpio())) {
    this->mod->hal->yield();
    if(this->mod->hal->millis() - start >= busyTimeout) {
