...
## shadow_channels_fuzzer.cpp

This fuzzer targets `shadowChannelsTryDecrypt` (src/mesh/ShadowChannels.cpp), the Multi-SF bridge's
preset-agnostic decryption path. Any radio packet whose wire channel-hash byte doesn't match any
configured channel on the node falls through to this function, making it a direct attacker-reachable
surface.

The fuzzer encodes each input as:

    [0]       channel hash byte
    [1..4]    from  (little-endian uint32)  - half of the AES-CTR nonce
    [5..8]    id    (little-endian uint32)  - other half of the nonce
    [9..N-1]  encrypted payload (<= sizeof(MeshPacket::encrypted.bytes))

and calls `shadowChannelsTryDecrypt(&p)` once per input. The function iterates the static preset-name
table (`LongFast`, `ShortSlow`, `ShortFast`, `LongMod`, `ShortTurbo`, ...), stages the default PSK
into the CryptoEngine, AES-CTR-decrypts the payload in place, and runs `pb_decode_from_bytes` on the
result. Any memory-safety issue in the scratch copy, nonce setup, or protobuf decode path should
surface here.

Seed corpus (`shadow_channels_fuzzer_seed_corpus.py`) includes one input per known preset hash so the
fuzzer reaches the decrypt body on its first generation rather than guessing an 8-bit hash value
blindly.

The harness requires `MESHTASTIC_MULTI_SF_BRIDGE=1` at build time; the ClusterFuzzLite pre-script
appends this define unconditionally so the shadow channels translation unit is compiled into the
fuzzer binary.
